...
{-
render ::
  forall drawWindow node.
  (Common.CommonTypes.DocNode node,
   gtk-0.12.5.7:Graphics.UI.Gtk.Types.DrawableClass drawWindow) =>
  Rendering.RenTypes.Scale
  -> Bool
  -> Common.CommonTypes.DiffTree
  -> Arrangement.ArrTypes.Arrangement node
  -> (gtk-0.12.5.7:Graphics.UI.Gtk.Types.Window,
      drawWindow,
      gtk-0.12.5.7:Graphics.UI.Gtk.Types.GC)
  -> (gtk-0.12.5.7:Graphics.UI.Gtk.General.Structs.Point,
      Rendering.RenTypes.Size)
  -> Graphics.Rendering.Cairo.Internal.Render ()
-}
render ::
  forall drawWindow node.
  (Common.CommonTypes.DocNode node,
   DrawableClass drawWindow) =>
  Scale
  -> Bool
  -- -> DiffTree -- ++AZ++ should be DiffTreeArr
  -> DiffTreeArr -- ++AZ++ should be DiffTreeArr
  -> Arrangement node
  -> (Window,
      drawWindow,
      GC)
          ...
          then return ()
                    ...
                    do { let (x,y)=(lux+scaleInt scale x', luy+scaleInt scale y')
                       ; let childDiffTrees = case diffTree of
                                                DiffLeafArr c e          -> repeat $ DiffLeafArr c e
                                                DiffNodeArr c c' a b dts -> dts ++ repeat (DiffLeafArr False a)
...
-}
    ...
    (RowA id x' y' w' h' _ _ bColor arrs) ->
      do { let (x,y,w,h)=(lux+scaleInt scale x', luy+scaleInt scale y', scaleInt scale w', scaleInt scale h')
         ...
         ; let childDiffTrees = case diffTree of
                                  DiffLeafArr c a          -> repeat $ DiffLeafArr c a
                                  DiffNodeArr c c' a b dts -> dts ++ repeat (DiffLeafArr False a) -- in case there are too few dts
         ; sequence_ $ zipWith (renderArr oldClipRegion (wi,dw,gc) arrDb scale (x, y) viewedArea) childDiffTrees arrs
         }
{-     do { let (x,y,w,h)=(lux+scaleInt scale x', luy+scaleInt scale y', scaleInt scale w', scaleInt scale h')
        ; let childDiffTrees = case diffTree of
